...
const Dashboard = () => {
  ...
  // Función para mostrar notificación cuando se agrega al carrito (prioriza SW para PWA)
  const showCartNotification = async (phoneName, quantity = 1) => {
    if (!('Notification' in window)) return;
    if (Notification.permission !== 'granted') return;

    const title = '🛒 Producto agregado al carrito';
    const body = `${phoneName} (${quantity} unidad${quantity > 1 ? 'es' : ''}) se agregó a tu carrito`;
    const options = {
      body,
      icon: '/icon-192.svg',
      badge: '/icon-192.svg',
    ...
    try {
      // Preferir ServiceWorkerRegistration para que funcione en PWA/segundo plano
    ...
    // Fallback a Notification API directa si el SW no está listo
    try {
